...
public class VolunteerUnitTest
{
    ...
    public async Task Add_Pet_With_Empty_Pets_Return_Success_Result()
    {
        ...
        var petDetails = PetDetails.Create("test", "test", 1, 1, false, false, DateTime.Now.Date)
            .Value;
    ...
    [Fact]
    public async Task Update_Info_Should_Update_Pet_Success()
    {
        ...
        var petDetails = PetDetails.Create("test1", "test1", 2, 2, true, true, DateTime.Now.Date)
            .Value;
        var address = Address.Create("test1", "test1", 2, 2).Value;

        var phoneNumber = PhoneNumber.Create("89999999991").Value;

        var speciesBreed = SpeciesBreed.Create(SpeciesId.Create(Guid.Empty), Guid.Empty).Value;
        
        var updatePet = Pet.Create(volunteer.Pets[0].Id, name, description, petDetails, address, phoneNumber,
        ...
        volunteer.Pets[0].Name.Should().Be(updatePet.Value.Name);
        volunteer.Pets[0].PaymentDetails[0].Should().Be(updatePet.Value.PaymentDetails[0]);
    ...
    private Volunteer CreateVolunteerWithPets(int petsCount)
    {
        ...
        for (int i = 0; i < petsCount; i++)
        {
            ...
            pet.Value.AddPaymentDetails([PaymentDetails.Create("test","test").Value]);
