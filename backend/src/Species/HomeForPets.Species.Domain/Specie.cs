...
public class Specie : Core.Entity<SpeciesId>
{
    ...
    private Specie(SpeciesId speciesId, string name)
        : base(speciesId)
    {
        Name = name;
    }
    ...
    public static Result<Specie, Error> Create(SpeciesId speciesId, string name)
    {
        if (string.IsNullOrWhiteSpace(name) || name.Length > Constants.LOW_VALUE_LENGTH)
        {
            return Errors.General.ValueIsInvalid("Specie name");
        }

        return new Specie(speciesId, name);
    }
}